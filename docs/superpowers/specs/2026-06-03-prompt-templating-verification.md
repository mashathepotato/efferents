...
The acceptance gate from the spec — *"the Researcher proposes a synthetic_loss experiment and a row lands in runs.sqlite, no reference-domain hardcoding"* — is met.
...
- `tests/test_prompts_domain_agnostic.py` — zero reference-domain tokens across all framework prompts (executable coupling guard).
...
- Researcher opened campaign `c-132d4d4314` with a **domain-agnostic** hypothesis: *"What is the reproducible baseline synthetic_loss under the default config, and is the noise floor small enough for single-seed comparisons to be meaningful?"* — reasons entirely about `synthetic_loss`, zero reference-domain vocabulary.
...
- 9 Anthropic calls, ~$0.46. No `PromptRenderError`, no `no such column: seed`, no `reference_lab` import error.
...
Also: notifications now carry the active `lab_id` (`{lab_id} started/paused/stopped/digest/code committed`) instead of hardcoded `reference-lab`, so a user running several labs gets distinct notifications.

## Known limitations (deferred)

- **`_format_recent_runs` / `_saturation_report` in researcher.py still reference reference-domain run-table columns** (`raw_q`, `e_w1`, …). These are deterministic *user-message table builders* and the saturation heuristic — not the system prompts this slice templated. `_saturation_report` is already try/except-guarded (v0.1.1) so it no-ops on a domain-agnostic schema; `_format_recent_runs` renders empty cells for absent columns (degraded, not fatal). Generalizing these to read columns from `LabConfig.metrics` is the natural next slice (CLAUDE.md item 5 + 7, partially done).
...
- **Auto-reference_domain degradation:** this repo's framework prompts are now generic, so reference-lab runs degraded until it adds its own `<submission>/prompts/` override directory restoring the QFM/HEP prose. Intended direction; reference-lab's next session does this.

## Recommendation

Tag v0.1.2. The framework now runs a fully autonomous, domain-agnostic research cycle on a domain-agnostic lab — the core proof that the abstractions hold beyond the reference lab.
