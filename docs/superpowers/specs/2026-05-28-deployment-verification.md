...
The 3 skips are reference-domain-coupled tests with `@pytest.mark.skip(reason="reference-domain-specific; lives with reference-lab")` markers — see [tests/README.md](../../../tests/README.md). They were the 3 pre-existing failures from the CLAUDE.md handoff (62/65 carry over); now triaged.
...
✅ Validation passes against the domain-agnostic smoke lab. Proves the LabConfig loader works for any domain.
...
- `efferents.agents.executor.execute` was rewritten (domain-agnostic) to route through `_execute_run` + `_persist_run_result`. The legacy `reference_lab.run_from_config` path is no longer touched on new submissions.

**What I verified by hand on the smoke lab:**
- `efferents validate --submission examples/smoke-lab/` → OK
- `efferents start --submission examples/smoke-lab/` runs cleanly: daemon registers, orchestrator boots, Researcher fires (one Anthropic call cycle), Coder + Analyst cadence kicks in. No SQL crash on missing `runs` columns, no missing-`context/` crash, no `reference_lab.run_from_config` raise.
- The executor's stdout-JSON contract works end-to-end: on the daemon run where the Researcher's reference-domain-flavored proposal reached the executor, the subprocess was spawned with the right `cwd`, stderr was captured (it surfaced a `python: command not found` shell-PATH issue on the smoke lab — fixed by switching the smoke lab's `run_command` to `python3`), and the generic "no JSON on stdout" error was logged in the new format. With `python3` in the command, `examples/smoke-lab/src/stub_run.py` standalone returns a valid stdout-JSON payload with `synthetic_loss`.

**The remaining gap — Researcher prompt-templating (Phase B).** The Researcher's prompt is still calibrated for reference-domain — it proposes experiments with overrides like `data.raw_q`, `training.epochs`, expects metrics like `e_w1`, `radial_l2_log`, `active_frac_w1`. The smoke lab's `coefficient` knob is invisible to it. So while the executor pipeline is functional and the smoke lab's `stub_run.py` works in isolation, the Researcher won't *autonomously drive* a domain-agnostic lab to convergence until prompt overrides ship in v0.1.2+. That's the next deploy slice.

**Net for v0.1.1:** the three deployment-blocking infrastructure gaps are closed. The next deploy slice is the Researcher (and Coder) prompt-templating work, which lets the Researcher propose experiments shaped by `LabConfig.metrics` and `LabConfig.source` rather than the original domain-specific lab's design space.

## 6. reference-lab compatibility

The reference-lab repo is a separate clone (`~/Documents/reference-lab`) and not validated here. The Task-0 prep (`fix(executor): make reference_lab.run import lazy`) means efferents now imports cleanly without reference-lab installed; reverse compatibility — reference-lab continuing to use efferents — is documented as requiring reference-lab's `run.py` to start emitting the stdout-JSON contract (or alternatively, the dual-read fallback discussed in spec section 7).
...
- **Framework:** Phase A code is no longer reference-domain-coupled at the load-bearing decouple touchpoints. New labs supply their own `lab.yaml`; the framework loads it and exposes it via `lab.get_config()`.

## 8. Recommendation

Ship v0.1.0 with these caveats documented in `examples/smoke-lab/README.md` and `skills/intake.md`. The entry-flow side of the deployment is complete and tested. The orchestrator side (real research cycles against the smoke lab) needs the two follow-ups in §5 before domain-agnostic users can run a full lab — that's a Phase B priority.
