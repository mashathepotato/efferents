...
- Rewrite the 9 reference-domain-coupled framework prompts to be domain-agnostic + templated. (`reviewer_neutral.md` and `reviewer_enthusiast.md` are already clean — no change.)
...
Out of scope:
- Auto-reference_domain's own `prompts/` override directory (lives in the reference-lab repo; its next session adds it).
...
Lines per framework prompt matching the reference-domain token set
(`e_w1|raw_q|aug_depth|active_frac|radial_l2|QFM|jet|amp_ratio|wallpaper|reference_lab|gen_max`):
...
**Auto-reference_domain path:** reference-lab's next session adds its own `prompts/` override dir restoring the QFM/HEP/amp_ratio prose. No framework change needed then.
...
- Replace the fixed three-metric reference-domain vector with `{panel_metrics_block}` (the lab's actual panels).
...
**Medium (coder, writer):**
- `coder.md`: `reference_lab/X.py` → `{source_dir}`; `python -m reference_lab.run` → `{smoke_command}`. Drop reference-domain-specific "don't break the diffusion math" cautions; keep generic "preserve the run contract, minimal diffs."
- `writer.md`: reference-domain metric names in the results-section guidance → `{headline_metric}` / `{panel_metrics}`. The 5-section paper structure is already generic.
...
**Acceptance bar:** `grep -iE "e_w1|raw_q|aug_depth|active_frac|radial_l2|qfm|jet|amp_ratio|wallpaper|reference_lab|gen_max" efferents/agents/prompts/*.md` returns zero matches.
...
- Grep every framework prompt for the reference-domain token set; assert zero matches. Executable form of §3's acceptance bar; fails CI on regression.

**Existing tests:** Researcher/Coder/Analyst/Writer tests driven with `FakeAnthropic` that assert on reference-domain strings in the rendered system prompt get genericized or moved to `tests/lab_reference/`. Expect a handful of touch-ups.

**Acceptance gate (extends `tests/integration/test_smoke_lab_e2e.py`):**
- With templated prompts the Researcher proposes a `coefficient`-varying experiment, the executor runs `stub_run.py`, and a `synthetic_loss` row lands in `runs.sqlite`. The existing "≥1 row with non-null synthetic_loss within 90s" assertion becomes genuinely achievable (it was blocked purely by reference-domain-flavored prompts).
...
- Auto-reference_domain's `prompts/` override dir (its repo).
