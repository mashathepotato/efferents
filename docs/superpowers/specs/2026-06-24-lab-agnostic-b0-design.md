...
their own (domain-agnostic) labs cleanly. This is the **precondition** (CLAUDE.md hard
...
1. **`analyst.py` (digests) — LIVE.** Hardcodes reference-domain metric/param columns
...
2. **`progress.py` (static `lab/progress.html`) — LIVE** (refreshed by
   `analyst.write_digest` → `write_progress`). Hardcodes reference-domain columns in
...
3. **`writer.py` legacy LaTeX/figure path — DEAD for the platform.**
   ...
   reference_lab-specific and only reachable via `write_once` (and the legacy
...
paths onto it, and delete the dead reference_lab path.** Approach 1 (shared helper)
...
- A domain-agnostic lab (e.g. the smoke lab, headline `synthetic_loss`/min) produces
  correct digests and a correct `progress.html` with no reference-domain column references.
- A `direction: max` lab (e.g. accuracy) gets correct "best run" and
  improvement/plateau detection.
- No `reference_lab`/reference-domain column names remain in `analyst.py`, `progress.py`, or
  `writer.py`.
- Full test suite green, with reference-domain-encoding tests rewritten to the generic /
...
- `_scored_sample_runs`: a run is "scored" iff
  `metrics_view.headline_value(row) is not None`. Drop the reference-domain
...
- HTML render functions (`_render_card`, `_render_run_tile`,
  `_render_architectures`, etc.): replace all reference-domain column references with the
  headline metric, the configured panels, and a generic key/value rendering of
  the remaining discovered columns. `_render_architectures` (reference-domain "architecture"
...
### `writer.py` (+ `__main__.py`) — delete the dead reference_lab path
...
- Delete: `write_once`, `regenerate_data_efficiency_figure`,
  ...
  helper used only by these (e.g. reference-domain figure utilities). Remove `WriterPaths`
...
Pytest. The risk is the existing reference-domain-encoding tests; they are rewritten to the
...
- **Update** `test_analyst_grouping.py`, `test_analyst_epsilon.py`,
  ...
  not reference-domain columns. Keep a `direction: max` case.
...
- **conftest.py:** the pre-migration `fresh_runs_db` (reference-domain columns) stays for
...
- **Acceptance:** full suite green; and a manual check that
  ...
  `progress.html` with synthetic_loss panels and no reference-domain columns.
