...
optimum (threshold 0.65 → val_f1 0.8889), and writes an auditable research memo
with an evidence table linking every claim to a run id and a train/eval log —
all on local compute, no API key. The automated audit is explicitly not labeled
as scientific peer review.
